...
impl Symtab {
    ...
    pub fn into_program_info(self, main_func: Option<FuncDefId>) -> super::ProgramInfo {
        super::ProgramInfo {
    ...
    /// Sets the currently analyzed function.
    ///
    /// This is used for forward referencing: when a function was already defined
    /// in the pre-pass and needs to be re-entered in the main pass.
    pub fn set_current_func(&mut self, func_id: FuncDefId) {
    ...
    pub fn resolve(&self, ident: &str) -> Result<ast::DefId, ResolveError> {
        ...
        Err(ResolveError(ast::Ident(ident.to_owned())))
    }
    ...
    fn define(&mut self, ident: ast::Ident, analysis: DefInfo) -> Result<ast::DefId, DefineError> {
        ...
        match innermost_scope.entry(ident) {
            ...
            Entry::Occupied(entry) => Err(DefineError(entry.key().clone())),
        }
    }

    /// Defines a new function in the global scope.
    ///
    /// The parameter types are collected from the `params` slice and stored
    /// in [`FuncInfo::param_types`] so they are available for type-checking
    /// forward calls without having to register parameters as local variables.
    pub fn define_func(
        &mut self,
        ident: ast::Ident,
        ...
        let param_types: Vec<ast::DataType> = params.iter().map(|p| p.data_type).collect();
...
/// An error returned by [`Symtab::define`] if the symbol is already
/// defined in the innermost scope.
pub struct DefineError(pub ast::Ident);

/// An error returned by [`Symtab::resolve`] if the symbol cannot be found.
pub struct ResolveError(pub ast::Ident);
...
pub struct FuncInfo {
    ...
    /// The data types of the parameters, stored separately so they are
    /// available for type-checking forward calls without populating
    /// [`local_vars`](Self::local_vars).
    pub param_types: Vec<ast::DataType>,
